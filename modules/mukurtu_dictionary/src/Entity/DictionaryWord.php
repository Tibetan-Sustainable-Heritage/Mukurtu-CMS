...
class DictionaryWord extends Node implements DictionaryWordInterface, CulturalProtocolControlledInterface, BundleSpecificCheckCreateAccessInterface, MukurtuDraftInterface {
  ...
  public static function bundleFieldDefinitions(EntityTypeInterface $entity_type, $bundle, array $base_field_definitions)
  {
    ...
    $definitions['field_definition'] = BaseFieldDefinition::create('text_long')
      ...
      ->setDescription(t('A longer definition or description of the entry. </br>This HTML field can support rich text and embedded media assets using the editing toolbar.'))
      ->setDefaultValue('')
      ->setCardinality(1)
      ->setRequired(FALSE)
      ->setRevisionable(TRUE)
    ...
    $definitions['field_recording'] = BaseFieldDefinition::create('entity_reference')
      ...
      ->setDescription(t('Audio recordings of the entry being spoken, usually on its own. Multiple recordings may be used to represent different types of speakers (eg: speakers of different ages, genders, accents, or dialects), or different forms the entry can take. </br>Recordings can be assigned a different cultural protocol from the dictionary word to allow differential access to the recordings and metadata. </br>Select "Add media" to select or upload audio files. Supported file formats: MP3, M4A, WAV, AAC, OGG. </br>Note that the audio file itself includes a contributor field that can be used to record and display the name of the speaker.'))
      ->setSettings([
        'target_type' => 'media',
        'handler' => 'default:media',
        'handler_settings' => [
    ...
    $definitions['field_word_origin'] = BaseFieldDefinition::create('string')
      ...
      ->setDescription(t('Information about the history or etymology of the entry. Examples include the origin language of a borrowed word or the date the word came into the language. </br>Maximum 255 characters.'))
      ->setSettings([
        'max_length' => 255,
      ])
      ->setDefaultValue('')
    ...
    $definitions['field_word_type'] = BaseFieldDefinition::create('entity_reference')
      ...
      ->setDescription(t('Word types may include parts of speech, syntactic or grammatical categories, or any other relevant system to classify entries.	</br>Include as many word types as needed. Select from existing word types or add new ones.'))
      ->setSettings([
        'target_type' => 'taxonomy_term',
        'handler' => 'default:taxonomy_term',
        'handler_settings' => [
